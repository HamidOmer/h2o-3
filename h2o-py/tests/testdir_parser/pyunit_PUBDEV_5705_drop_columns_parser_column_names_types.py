...
def import_with_column_names_types():
    ...
    pathHeader=pyunit_utils.locate("smalldata/airlines/allyears2k_headers.zip")

    # upload file with  no header but fixed column types.
    csvWithNoHeader = h2o.upload_file(pyunit_utils.locate("smalldata/airlines/allyears2k.csv"), header=-1)
    allNewColnames = csvWithNoHeader.names
    allNewTypeDict = csvWithNoHeader.types
    pathNoHeader=pyunit_utils.locate("smalldata/airlines/allyears2k.csv")
